    ...
    public function generate()
    {
        ...
        foreach ($this->varValue as $key => $fieldValue) {
            ...
            for ($i = 0; $i < \count($fields); ++$i) {
                if ($i === $fields['type']) {
                ...
                } elseif ($i === $fields['scripts'] && isset($fieldValue[$i])) {
                    ...
                    $return .=
                        $this->createInputFieldAndLabel($key, $i, 'deleteValOnCopy', $tabindex, specialchars($fieldValue[$i]), [$i]);
                } elseif (isset($fieldValue[$i])) {
...
//                $class = ($button == 'up' || $button == 'down') ? ' class="button-move"' : '';
                if ('drag' === $button) {
                    $return .= Image::getHtml(
                        ...
                        $GLOBALS['TL_LANG']['MSC']['move'].'" style="top:3px;"'
                    );
                    $return .= "\n\n";
                } else {
                    $return .= Image::getHtml(
    ...
    public function createInputFieldAndLabel($key, $i, $classes, $tabindex, $value, $wizard = [])
    {
    ...
    private function createPageWizard($key, $i, $value)
    {
        ...
        return Backend::getDcaPickerWizard(['fieldType' => 'radio'], Input::get('table'), '', 'venoList[0][1]');
